...
Front::Front(int id_, int start_, int size_) : id(id_), start(start_), self_size(size_) {
    nbr_size = 0;
...
MF::MF(vector<Front*> fronts_, VectorXi perm_, VectorXi invperm_) : fronts(fronts_), perm(perm_), invperm(invperm_) {};
void MF::factorize() { 
...
void MF::extract(MatrixXd& A) {
    for(auto f : fronts) {
        f->extract(A);
    }
}
void MF::print_ordering(string ordering_file, string neighbors_file, string tree_file) {
